...
function get_cookie(){
    return new Promise((resolve, reject) => {
        var url = 'http://bdxxtt.juwo.com/wapads/index.php'
        rp({url: url, jar: j})
        .then(function (body) {
...
async function login_byCookie(...func){
    await get_cookie()
    var url = 'http://bdxxtt.juwo.com/ajax/ajax.php'
    var options = {  
        url:url, 
        jar: j,
        method:"POST",
...
async function get_data(cb){
    ...
    var options = {  
        ...
        form: {
            'PHPSESSID': 'a7bs23kjntroifhreopb7cbes7',
            'qweds': 1
        }
    }
    options.headers.Cookie = cookie_string
    ...
    if (html) {
        ...
        tableList.find('tbody tr').each(function(item) {
            ...
            dataList.push({
                ...
                accounts: that.find('td').eq(2).text(),
                consume: that.find('td').eq(3).text(),
